  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const first = Array.isArray(projJson.projects) && projJson.projects.length
          ? projJson.projects[0]
          : null
        if (!first) {
          if (!cancelled) {
        ...
        // load compile result for first.project_id
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
      <section className="bg-white shadow-card rounded-xl p-4">
        ...
        {loadError && (
          <div className="mt-4 text-xs text-red-600 border border-red-200 bg-red-50 rounded p-2">
            {loadError}
          </div>
        )}
      </section>
      ...
      <section className="bg-white shadow-card rounded-xl p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            {pages.length === 0 ? (
              <p className="text-gray-400 text-sm italic">No pages yet</p>
            ) : (
          ...
          <div>
            ...
            {forms.length === 0 ? (
              <p className="text-gray-400 text-sm italic">No forms yet</p>
            ) : (
              <div className="space-y-3">
                {forms.map((f) => (
                  <EditableElementCard
          ...
          <div>
            ...
            {buttons.length === 0 ? (
              <p className="text-gray-400 text-sm italic">No buttons yet</p>
            ) : (
              <div className="space-y-3">
                {buttons.map((b) => (
                  <EditableElementCard
